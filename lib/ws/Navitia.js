...
export default class Navitia {
    constructor() {
        this.token = process.env.NAVITIA_TOKEN;
    }
    ...
    getPlaces(query, types = []) {
        let url = `http://api.navitia.io/v1/coverage/fr-idf/places?q=${encodeURIComponent(query)}`;
        types.forEach(type => {
            url += `&type[]=${type}`;
        });
        return this.get(url);
    ...
    getJourneys(from, to, datetime, datetimeRepresents) {
        let url = `http://api.navitia.io/v1/coverage/fr-idf/journeys?from=${from}&to=${to}&datetime=${toISODatetime(datetime)}`;
        if (datetimeRepresents) {
            url += `&datetime_represents=${datetimeRepresents}&forbidden_uris[]=network:OIF:56`;
        }
        return this.get(url);
    }
